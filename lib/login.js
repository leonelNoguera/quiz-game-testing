  ...
  )
  {
    ...
    if (newUser)
    {
      game.users.push({
        ...
        'voteForAnAnswer' : data['voteForAnAnswer'], 
        'voteLeader' : data['voteLeader'], 
        'votesReceived' : 0, 
        'connected' : true
      });//Pendiente ver si es necesario el data para 'voteForAnAnswer' y 'voteLeader'.
      game.userIds.push(socket.id);
    }
    var teamIndex = -1;
    ...
    if (teamIndex == -1)
    {
      game.teams.push({
        ...
        'users' : [{'userName' : data['userName'], 'userSurname' : data['userSurname'], 'voteForAnAnswer' : false, 'voteLeader' : false, 'votesReceived' : 0, 'connected' : true}], 
        'sendedQuestions' : {
          'area1' : [], 
          'area2' : [], 
          'area3' : []
...
function updateSocketId(socketId, userName, userSurname)
{
  for (var i = 0; i < game.users.length; i++)
  {
    ...
    {if (game.userIds[i] != socketId){console.log('Actualizando socket.id');}
      game.userIds[i] = socketId;
    }
  }
}
