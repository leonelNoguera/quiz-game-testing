...
function voteLeader(data, socket)
{
  ...
  if (teamIndex != -1)
  {
    game.teams[teamIndex]['newLeader'] = false;
    ...
    {//Ver si hay algún ganador.
      ...
      {//Hay un ganador.
        ...
        {
            if (game.teams[teamIndex]['users'][k]['connected'] && (!game.teams[teamIndex]['users'][k]['vote']))
            {
              allUsersVoted = false;
            }
        }
        if (allUsersVoted)
        {
          ...
          for (var j = 0; j < game.teams[teamIndex]['users'].length; j++)
          {
            ...
            {//Pendiente evitar que se reinicien las preguntas luego de elejir a otro lider porque el anterior se desconectó.
              ...
              game.teams[teamIndex]['users'][j]['status'] = 'useTheWheel';console.log('Wheel para ' + game.teams[teamIndex]['users'][j]['userName'] + ' ' + game.teams[teamIndex]['users'][j]['userSurname']);
      ...
      {console.log('voting, line 119: data == ' + JSON.stringify(data));
        for (var k = 0; k < game.teams[teamIndex]['users'].length; k++)
        {
          game.teams[teamIndex]['users'][k]['vote'] = false;
          game.teams[teamIndex]['users'][k]['votesReceived'] = 0;
        }
        /*data['teams'] = game.teams;//Pendiente revisar esto.
        for (var k = 0; k < game.teams[teamIndex]['users'].length; k++)
        {
          data['userName'] = game.teams[teamIndex]['users'][k]['userName'];
          data['userSurname'] = game.teams[teamIndex]['users'][k]['userSurname'];
...
{console.log('voting, line 138: data == ' + JSON.stringify(data));//game.teams[i]['sendedQuestions'].push(game.questions['area1'][j]['question']);
  for (var i = 0; i < game.teams.length; i++)
  {
    if (game.teams[i]['teamName'] == data['teamName'])
    {
      ...
      {//Tiene que decidir el líder.
        game.teams[i]['status'] = 'leaderVotation';
...
function leaderVotation(data, socket)
{
  for (var i = 0; i < game.teams.length; i++)
  {
    if (game.teams[i]['teamName'] == data['teamName'])
    {
      ...
      game.teams[i]['status'] = 'detailedExplanationOfAnswers';

      game.teams[i]['score'] = data['score'];
      game.teams[i]['finalAnswer'] = data['finalAnswer'];
      game.teams[i]['bestAnswerScore'] = data['bestAnswerScore'];
      game.teams[i]['options'] = data['options'];
      game.teams[i]['topic'] = data['topic'];
      game.teams[i]['question'] = data['question'];
      data['scoreArea1'] = game.teams[i]['scoreArea1'];
...
{console.log('voting, line 302: data == ');console.log(data);
  for (var i = 0; i < game.teams.length; i++)
  {
    if (game.teams[i]['teamName'] == data['teamName'])
    {
      //game.teams[i]['status'] = 'personalEvaluation';
      for (var j = 0; j < game.teams[i]['sendedQuestions']['area1'].length; j++)
      {
        if (game.teams[i]['sendedQuestions']['area1'][j]['question'] == data['question'])
        {
