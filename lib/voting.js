  ...
  if (teamIndex != -1)
  {
    for (var i = 0; i < game.teams[teamIndex]['users'].length; i++)
    {
      if ((game.teams[teamIndex]['users'][i]['userName'] == data['userNameVoting']) && 
        ...
        (!game.teams[teamIndex]['users'][i]['voteForAnAnswer']))
      {
        game.teams[teamIndex]['users'][i]['voteForAnAnswer'] = true;
        vote = true;
    ...
    {//Ver si ya votaron todos en el team.
      if ((game.teams[teamIndex]['users'][i]['connected']) && (!game.teams[teamIndex]['users'][i]['voteForAnAnswer']))
      {
      ...
      {//Hay un ganador.
        ...
        for (var i = 0; i < game.users.length; i++)
        {
          ...
          for (var j = 0; j < game.teams.length; j++)
          {
              ...
              {
                  if (game.teams[j]['users'][k]['connected'] && (!game.teams[j]['users'][k]['voteForAnAnswer']))
                  {
                    allUsersVoted = false;
                  }
        ...
        {//Se reinicia la votación. Puede ser necesario cuando se desconecta alguno durante la elección del lider.
          if (game.teams[j]['teamName'] == data['teamName'])
          {
            ...
            {//Pendiente revisar esto.
              game.teams[j]['users'][k]['voteLeader'] = false;
